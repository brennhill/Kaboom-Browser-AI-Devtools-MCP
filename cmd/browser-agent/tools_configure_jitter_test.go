...
func TestToolConfigureActionJitter_SetValue(t *testing.T) {
	...
	resp := configurehandler.HandleActionJitter(h, req, args)
...
func TestToolConfigureActionJitter_MaxClamp(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := JSONRPCRequest{JSONRPC: "2.0", ID: 1}
			resp := configurehandler.HandleActionJitter(h, req, json.RawMessage(tt.args))
...
func TestToolConfigureActionJitter_PartialUpdate(t *testing.T) {
	...
	resp1 := configurehandler.HandleActionJitter(h, req, args1)
	...
	resp2 := configurehandler.HandleActionJitter(h, req, args2)
...
func TestToolConfigureActionJitter_EmptyJSON_ReturnsCurrentValues(t *testing.T) {
	...
	resp2 := configurehandler.HandleActionJitter(h, req, json.RawMessage(`{}`))
	result2 := parseToolResult(t, resp2)
	if result2.IsError {
...
func TestToolConfigureActionJitter_NilArgs_ReturnsDefaults(t *testing.T) {
	...
	req := JSONRPCRequest{JSONRPC: "2.0", ID: 1}
	resp := configurehandler.HandleActionJitter(h, req, nil)
	result := parseToolResult(t, resp)
	if result.IsError {
...
func TestToolConfigureActionJitter_InvalidJSON_LenientFallback(t *testing.T) {
	...
	req := JSONRPCRequest{JSONRPC: "2.0", ID: 1}
	resp := configurehandler.HandleActionJitter(h, req, json.RawMessage(`{bad json`))
