...
func (e *gateTestEnv) simulateConnection(t *testing.T) {
	...
	newSyncHandler(e.capture).HandleSync(httptest.NewRecorder(), httpReq)
}
...
func TestNavigate_ExtDisconnected_FastFail(t *testing.T) {
	...
	resp := env.handler.browserActions.Handle("navigate", req, args)
...
func TestExecuteJS_CSP_MainWorld_FastFail(t *testing.T) {
	...
	resp := env.handler.browserActions.Handle("execute_js", req, args)
...
func TestClick_ExtDisconnected_FastFail(t *testing.T) {
	...
	resp := env.handler.domActions.HandleDOMPrimitive(req, args, "click")
