...
func HandleLinkValidation(req mcp.JSONRPCRequest, args json.RawMessage, version string) mcp.JSONRPCResponse {
	...
	if len(params.URLs) == 0 {
		return fail(req, mcp.ErrMissingParam, "Required parameter 'urls' is missing or empty", "Provide an array of URLs to validate")
	}
	...
	if len(validURLs) == 0 {
		return fail(req, mcp.ErrInvalidParam, "No valid HTTP/HTTPS URLs provided", "URLs must start with http:// or https://", mcp.WithParam("urls"))
	}
