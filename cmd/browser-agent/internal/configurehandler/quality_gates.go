// quality_gates.go — Handles configure(what="setup_quality_gates").
// Scaffolds .kaboom.json and kaboom-code-standards.md in the target directory and installs the
// Claude Code hooks. Isolates the scaffolding logic from the configure router so the package-main
// entry point stays a thin delegator.
...
const (
	kaboomHookQualityGate    = "kaboom-hooks quality-gate"
	kaboomHookCompressOutput = "kaboom-hooks compress-output"
	kaboomHookSessionTrack   = "kaboom-hooks session-track"
	kaboomHookBlastRadius    = "kaboom-hooks blast-radius"
	kaboomHookDecisionGuard  = "kaboom-hooks decision-guard"
)

// HandleSetupQualityGates handles configure(what="setup_quality_gates").
// Creates .kaboom.json and kaboom-code-standards.md in the target directory.
func HandleSetupQualityGates(d Deps, req mcp.JSONRPCRequest, args json.RawMessage) mcp.JSONRPCResponse {
	var params struct {
	...
	if err != nil {
		return fail(req, mcp.ErrInvalidParam, "Invalid target_dir: "+err.Error(), "Provide a valid directory path", mcp.WithParam("target_dir"))
	}
	...
	if !strings.HasPrefix(absTarget+string(filepath.Separator), absProject+string(filepath.Separator)) && absTarget != absProject {
		return fail(req, mcp.ErrPathNotAllowed,
	...
	if _, err := os.Stat(configPath); err == nil {
		...
		if err != nil {
			return fail(req, mcp.ErrInternal, "Failed to marshal config: "+err.Error(), "Internal error — do not retry")
		}
		cfgJSON = append(cfgJSON, '\n')
		if err := os.WriteFile(configPath, cfgJSON, 0644); err != nil {
			return fail(req, mcp.ErrInternal, "Failed to write "+hook.KaboomConfigFile+": "+err.Error(), "Check file system permissions")
		}
	}
	...
	if codeStandardsRef == hook.DefaultCodeStandardsFile {
		...
		if _, err := os.Stat(standardsPath); err != nil {
			if err := os.WriteFile(standardsPath, []byte(defaultCodeStandardsContent), 0644); err != nil {
				return fail(req, mcp.ErrInternal, "Failed to write "+hook.DefaultCodeStandardsFile+": "+err.Error(), "Check file system permissions")
			}
