...
//     only when the file holds more than logCompactionFactor*maxEntries entries
...
// newAsyncLogStoreForTest builds a LogStore with a running worker, mirroring
// the wiring in NewServer (worker started before any entries are added).
func newAsyncLogStoreForTest(t *testing.T, maxEntries int) (*LogStore, string) {
	t.Helper()
	logFile := filepath.Join(t.TempDir(), "async.jsonl")
	ls := NewLogStore(logFile, maxEntries, func(string) {})
	go ls.asyncLoggerWorker()
	return ls, logFile
}
...
func TestLogStoreSteadyStatePostsAppendInsteadOfRewriting(t *testing.T) {
	...
	for i := 0; i < posts; i++ {
		ls.addEntries([]LogEntry{{"level": "info", "message": fmt.Sprintf("entry-%d", i)}})
	}
	ls.shutdownAsyncLogger(2 * time.Second)
...
// once the file exceeds logCompactionFactor*maxEntries entries.
func TestLogStoreCompactionRewritesAfterThreshold(t *testing.T) {
	...
	const posts = logCompactionFactor*maxEntries + 1
	...
	if rewrites := ls.fileRewriteCount.Load(); rewrites != 1 {
		t.Fatalf("full-file rewrites = %d, want exactly 1 (compaction after crossing %d entries)", rewrites, logCompactionFactor*maxEntries)
	}
...
func TestLogStoreClearEntriesTruncatesFileAndResetsCount(t *testing.T) {
	...
	if got := ls.getEntryCount(); got != 0 {
		t.Fatalf("getEntryCount() = %d, want 0 after clear", got)
	}
...
func TestLogStoreConcurrentAddsProduceValidFile(t *testing.T) {
	...
	for g := 0; g < goroutines; g++ {
		...
		go func(g int) {
			...
			for i := 0; i < addsPerGoroutine; i++ {
				ls.addEntries([]LogEntry{{"level": "info", "message": fmt.Sprintf("g%d-%d", g, i)}})
			}
		}(g)
	}
	wg.Wait()
	...
	if got := ls.getEntryCount(); got != maxEntries {
		t.Fatalf("getEntryCount() = %d, want %d", got, maxEntries)
	}
