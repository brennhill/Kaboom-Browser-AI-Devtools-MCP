#!/bin/bash
# smoke-test.sh — Human smoke test for Kaboom MCP.
...
#
# Usage:
#   bash scripts/uat/runners/smoke-test.sh                    # default port 7890
#   bash scripts/uat/runners/smoke-test.sh 7890               # explicit port
#   bash scripts/uat/runners/smoke-test.sh --start-from 12    # resume from module 12
#   bash scripts/uat/runners/smoke-test.sh 7890 --start-from 07
set -euo pipefail
...
while [ $# -gt 0 ]; do
    case "$1" in
        --help|-h)
            echo "Usage: bash scripts/uat/runners/smoke-test.sh [PORT] [OPTIONS]"
            echo ""
...
if lsof -ti :"$PORT" >/dev/null 2>&1; then
    ...
    if [ -n "$existing_ver" ]; then
        ...
        echo "  Port $PORT occupied by non-Kaboom process. Killing..." >&2
