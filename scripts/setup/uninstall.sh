...
#
# PURPOSE:
# Reverses every artifact created by scripts/setup/install.sh and by
...
# docs/architecture/runtime/uninstall-and-cleanup.md for the artifact map.
#
# USAGE:
...
#   ./scripts/setup/uninstall.sh                # interactive (prompts before removing)
#   ./scripts/setup/uninstall.sh --dry-run      # show what would be removed
#   ./scripts/setup/uninstall.sh --keep-data    # keep logs/recordings/project state
#
# Windows users: use scripts/setup/uninstall.ps1 instead.
...
case "$OS" in
    ...
    mingw*|cygwin*)
        echo -e "${YELLOW}On Windows, run scripts/setup/uninstall.ps1 from PowerShell instead.${NC}"
        exit 1
        ;;
    *) PLATFORM="other" ;;
esac
...
if [ "$DRY_RUN" != "1" ] && [ "$ASSUME_YES" != "1" ]; then
    if [ -t 0 ]; then
        ...
        echo -e "  curl -sSL https://raw.githubusercontent.com/brennhill/Kaboom-Browser-AI-Devtools-MCP/STABLE/scripts/setup/uninstall.sh | bash -s -- --yes"
        exit 1
    fi
fi
...
echo -e "  curl -sSL https://raw.githubusercontent.com/brennhill/Kaboom-Browser-AI-Devtools-MCP/STABLE/scripts/setup/install.sh | bash"
