...
test('mutation runner reports a killed semantic regression', () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      minimum_score: 100,
      cases: [
        { id: 'positive_boundary', file: 'sample/sample.go', package: './sample', from: 'value > 0', to: 'value >= 0' }
      ]
    })
  )
...
test('mutation runner fails its gate and records a surviving regression', () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      minimum_score: 100,
      cases: [
        { id: 'untested_negative', file: 'sample/sample.go', package: './sample', from: 'value > 0', to: 'value != 0' }
      ]
    })
  )
