...
if ./scripts/quality/verification/validate-architecture.sh > /dev/null 2>&1; then
...
if [ "$ERRORS" -eq 0 ]; then
    ...
    echo "Run: ./scripts/quality/verification/verify-no-regressions.sh for detailed diagnostics"
    exit 1
fi
