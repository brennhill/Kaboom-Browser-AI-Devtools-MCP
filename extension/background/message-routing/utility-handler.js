...
async function pushChat(message, pageUrl, tabId, respond) {
    try {
...
async function requestAudit(pageUrl, serverUrl, respond) {
...
export function createUtilityMessageHandler(deps) {
    return {
        feature: 'utility',
        handle(message, sender, sendResponse) {
            switch (message.type) {
                ...
                case 'open_terminal_panel':
                    openTerminalSidePanel(message.tab_id ?? sender.tab?.id)
                        .then(sendResponse)
...
//# sourceMappingURL=utility-handler.js.map