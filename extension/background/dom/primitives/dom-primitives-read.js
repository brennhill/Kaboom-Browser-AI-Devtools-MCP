    ...
    function buildActionHandlers(node) {
        return { get_text: () => {
                if (options.structured && node instanceof HTMLElement) {
                    ...
                    return { success: true, action, selector, sections, section_count: sections.
                            length };
                }
                const text = node instanceof HTMLElement ? node.innerText : node.textContent;
                if (text === null || text === void 0) {
                    return { success: true, action, selector, value: text, reason: "\
no_text_content", message: "Resolved text content is null" };
                }
                return { success: true, action, selector, value: text };
            }, get_value: () => {
                if (!("value" in node))
                    return domError("\
no_value_property", `Element has no value property: ${node.tagName}`);
                const value = node.value;
                if (value === null || value === void 0) {
                    return { success: true, action, selector,
                        value, reason: "no_value", message: "Element value is null" };
                }
                ...
                return {
                    success: true, action, selector, value
                };
                ...
                return { success: false, action, selector: "", error: "\
text_not_found" };
            }, wait_for_absent: () => {
                if (!selector) {
                    return { success: false, action, selector: "", error: "missing_selector", message: "selector is required for wait_for\
_absent" };
                }
                ...
                return { success: false, action, selector, error: "element_still\
_present" };
            } };
    }
