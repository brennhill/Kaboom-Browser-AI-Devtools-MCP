    ...
    function resolveActionTarget() {
        ...
        if (nthParam !== undefined && nthParam !== null) {
            ...
            if (resolvedIndex < 0 || resolvedIndex >= candidates.length) {
                return {
                    error: domError('nth_out_of_range', `nth=${nth} is out of range — selector matched ${candidates.length} element(s). Use nth 0..${candidates.length - 1} or -1..-${candidates.length}.`)
                };
            }
            return {
