...
export function domPrimitiveOverlay(action, options) {
    ...
    function isHighZIndexOverlayCandidate(el) {
        if (!(el instanceof HTMLElement))
            return false;
        const style = getComputedStyle(el);
    ...
    function findTopmostOverlay() {
        ...
        for (let i = 0; i < allElements.length; i++) {
            ...
            if (isHighZIndexOverlayCandidate(el))
                candidates.push(el);
        }
    ...
    function referencesExtensionOrigin(src) {
        return src.startsWith('chrome-extension://') || src.startsWith('moz-extension://');
    }
    function extensionResourceInChildren(root) {
        ...
        for (let i = 0; i < resources.length; i++) {
            ...
            if (referencesExtensionOrigin(src))
                return true;
        }
        return false;
    }
    function shadowHostLooksLikeExtension(host) {
        const hostTag = host.tagName?.toLowerCase() || '';
        if (extensionResourceInChildren(host))
            return true;
        if (EXTENSION_TAG_PREFIXES.some((prefix) => hostTag.startsWith(prefix)))
            return true;
        ...
        for (let k = 0; k < attrs.length; k++) {
            ...
            if (EXTENSION_ATTR_PATTERNS.some((pat) => attrName.startsWith(pat) || attrName.includes(pat)))
                return true;
        }
        return false;
    }
    function detectExtensionOverlay(el) {
        if (el instanceof HTMLElement && extensionResourceInChildren(el))
            return true;
        let node = el;
        while (node) {
            ...
            if (root && root !== document && root instanceof ShadowRoot) {
                ...
                if (host) {
                    if (shadowHostLooksLikeExtension(host))
                        return true;
                    node = host;
                    continue;
                }
    ...
    function findCloseButtonTarget(overlayElement) {
    ...
    function scoreDismissButton(btn) {
    ...
    function findDismissTextTarget(overlayElement) {
    ...
    function findDismissAttrTarget(overlayElement) {
