...
function isActionable(node) {
    if (node.states.some((state) => HIDDEN_STATES.has(state.toLowerCase())))
        return false;
...
function scoreName(name, queryTokens) {
    ...
    if (nameJoined === queryJoined)
        return { score: 1, why: 'exact accessible name' };
 ...
 */
export function rankAXCandidates(nodes, query) {
    const queryTokens = normalizeQuery(query);
    if (!nodes || queryTokens.length === 0)
        return [];
    const candidates = [];
 ...
 */
function statesFrom(properties) {
    const states = [];
 ...
 */
export async function fetchAXNodes(lease) {
    ...
    for (const item of raw) {
        ...
        if (typeof item.backendDOMNodeId !== 'number')
            continue;
        nodes.push({
 ...
 */
export async function resolveAXGeometry(lease, nodes) {
    const resolved = [];
    for (const node of nodes) {
        if (typeof node.backend_node_id !== 'number')
            continue;
        try {
            const reply = (await lease.send('DOM.getBoxModel', { backendNodeId: node.backend_node_id }));
            ...
            if (!points.every((n) => typeof n === 'number' && Number.isFinite(n)))
                continue;
