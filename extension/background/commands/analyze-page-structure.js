...
function pageStructureScript(useGlobals) {
    ...
    const detectFrameworksWithGlobals = () => {
        const found = [];
        const win = window;
        // Vue
    ...
    const detectFrameworksWithDOMHints = () => {
        const found = [];
    ...
    const collectScrollContainers = () => {
        ...
        for (const el of Array.from(skipScrollDetection ? [] : allElements)) {
            if (containers.length >= MAX_SCROLL_CONTAINERS)
                break;
            const htmlEl = el;
