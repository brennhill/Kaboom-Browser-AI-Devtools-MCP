...
// =============================================================================
function openTerminalFromMenu(tabId, logFn) {
...
async function handleControlMenuClick(tab, recordingHandlers, logFn) {
    try {
        const trackedTabId = (await readTrackedTab()).id;
        if (trackedTabId === tab.id) {
            ...
            await untrackTab(tab.id, async () => {
                if (recordingHandlers.isRecording())
...
function handleScreenshotMenuClick(tabId, logFn) {
    try {
...
async function handleRecordMenuClick(recordingHandlers, tab, logFn) {
    try {
...
async function handleActionRecordMenuClick(actionRecordingHandlers, tab, logFn) {
    try {
...
async function handleAnnotateMenuClick(tabId, logFn) {
    try {
