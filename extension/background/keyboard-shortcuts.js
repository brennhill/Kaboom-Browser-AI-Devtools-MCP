 ...
 */
export async function toggleActionSequenceRecording(handlers, tab, logFn) {
    if (!tab.id)
        return;
    ...
    if (handlers.isRecording()) {
        ...
        if (stopResult.status !== 'saved' && stopResult.status !== 'stopped') {
            sendTabToast(tab.id, 'Stop recording failed', stopResult.error || 'Could not stop action sequence recording', 'error', 3500);
            if (logFn)
                logFn(`Action recording stop failed: ${stopResult.error ?? 'unknown error'}`);
        }
        return;
    }
    ...
    if (startResult.status !== 'recording') {
        ...
        if (logFn)
            logFn(`Action recording start failed: ${startResult.error ?? 'unknown error'}`);
    }
}
