...
const syncManagerDeps = {
    ...
    getConnectionStatus,
    setConnectionStatus: (patch) => setConnectionStatus(patch),
...
function applyVersionState(health) {
    if (health.connected) {
        try {
...
function broadcastStatusUpdate() {
    ...
    chrome.runtime
        ...
        if (!isNoReceiverError(error))
            console.error(`${KABOOM_LOG_PREFIX} Error sending status update:`, error);
    });
}
export async function checkConnectionAndUpdate() {
    ...
    try {
        ...
        if (wasExtensionConnected !== health.extensionConnected && health.extensionConnected !== undefined) {
            debugLog(DebugCategory.CONNECTION, health.extensionConnected ? 'Extension heartbeat connected' : 'Extension heartbeat unavailable', { error: health.extensionError || null });
        }
