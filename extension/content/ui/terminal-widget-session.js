...
export function buildAIInitCommand(aiCommand) {
    ...
    const authCheck = isCodex
        ...
        : isClaude
            ...
            : '';
    const providerMarkers = ` case "$kaboom_execution_provider" in` +
    ...
    const apiPrompt = ` if [ "$kaboom_execution_provider" = api ]; then` +
        ...
        ` else ${aiCommand}; fi;`;
    return (`kaboom_execution_provider=unknown;` +
...
async function handleTerminalStartRejection(resp, onSandboxError) {
    const body = (await resp.json());
