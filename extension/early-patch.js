 ...
 */
function installSeededRandomness(seed) {
    const key = String(seed);
    if (window.__KABOOM_RANDOM_SEED_ACTIVE__ && window.__KABOOM_RANDOM_SEED__ === key)
        return true;
    const next = makeSeededRandom(key);
    const mathOK = safeAssignGlobal(Math, 'random', next);
    let cryptoOK = true;
    if (window.crypto && typeof window.crypto.getRandomValues === 'function') {
        const seededBytes = function (array) {
...
(function () {
    ...
    if (typeof window.__KABOOM_RANDOM_SEED__ === 'string')
        installSeededRandomness(window.__KABOOM_RANDOM_SEED__);
    // =========================================================================
