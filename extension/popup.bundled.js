...
(() => {
  ...
  function renderReport(report) {
    ...
    if (!elements.overall || !elements.checks)
      return;
    const waitingForAttachment = !report.ready_for_interaction && report.checks.every((check) => check.status === "pass" || check.name === "tracked_tab");
    const healthy = report.ready_for_interaction || waitingForAttachment;
    elements.overall.textContent = report.ready_for_interaction ? "Ready" : waitingForAttachment ? "Ready when attached" : "Needs attention";
    ...
    elements.checks.replaceChildren(...report.checks.map((check) => {
      ...
      if (check.recovery_attempt) {
        timelineParts.push(`Attempt ${check.recovery_attempt} \xB7 ${check.recovery_outcome ?? "pending"}`);
      }
      const recentEvents = (check.history ?? []).slice(-3).map((transition) => transition.event?.replaceAll("_", " ")).filter((event) => Boolean(event));
      if (recentEvents.length > 0)
        timelineParts.push(recentEvents.join(" \u2192 "));
      if (timelineParts.length > 0) {
        const timeline = document.createElement("div");
        timeline.className = "doctor-check-timeline";
        timeline.textContent = timelineParts.join(" \xB7 ");
  ...
  function isDoctorReport(value) {
    ...
    return typeof candidate.ready_for_interaction === "boolean" && typeof candidate.version === "string" && Array.isArray(candidate.checks);
  }
  async function refreshSystemDoctor(status, fetchImpl = fetch) {
    if (!status.connected) {
  ...
  // extension/popup/driven-tab-group-permission.js
  var DRIVEN_GROUP_TOGGLE_ID = "toggle-driven-tab-group";
  var DRIVEN_GROUP_ROW_ID = "driven-tab-group-row";
  ...
  function grant(toggle) {
    ...
    requested.catch((err) => {
      console.error("[KaBOOM!] The tabGroups permission request was rejected by Chrome", err);
      return false;
    }).then(() => reconcileToggle(toggle)).catch((err) => {
      console.error("[KaBOOM!] Could not settle the tab group toggle", err);
    });
  }
  function revoke(toggle) {
    const removal = typeof chrome?.permissions?.remove === "function" ? chrome.permissions.remove(tabGroupsPermission()) : Promise.resolve(false);
    removal.catch((err) => {
  ...
  function applyDrivenTabGroupToggle() {
