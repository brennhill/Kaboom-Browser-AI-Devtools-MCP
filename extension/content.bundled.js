...
(() => {
  ...
  // extension/content/page-telemetry.js
  ...
  function validatePageTelemetry(messageType, payload) {
    ...
    inspectValue(payload, /* @__PURE__ */ new WeakSet(), 0, inspection);
    return inspection.rejection ?? null;
  }

  // extension/content/window-message-listener.js
  var reportedTelemetryRejections = /* @__PURE__ */ new Set();
  ...
  var waitForWaterfallResponse = (response, cleanup) => withTimeoutAndCleanup(response, 5e3, { cleanup });
  function reportWaterfallBridgeLifecycle(lifecycle, requestId, reason = "") {
    ...
    try {
      void chrome.runtime.sendMessage({ type: "report_state_recovery", lifecycle, diagnostic }).catch((error) => console.warn("[KaBOOM!] Waterfall bridge diagnostic delivery failed:", errorMessage(error)));
    } catch (error) {
      console.warn("[KaBOOM!] Waterfall bridge diagnostic delivery failed:", errorMessage(error));
    }
  }
  function handleGetNetworkWaterfall(sendResponse, waitForResponse = waitForWaterfallResponse) {
    ...
    waitForResponse(deferred.promise, () => window.removeEventListener("message", responseHandler)).then((result) => {
      ...
      sendResponse({
        ...
        message: timedOut ? "Injected waterfall bridge did not respond before the deadline." : "Injected waterfall bridge rejected the response wait."
      });
    });
    return true;
  }
