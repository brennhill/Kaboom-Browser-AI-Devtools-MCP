...
| F2 | MED | 4 | `background/context-menus.ts` vs `popup/tab-tracking-api.ts` | Context-menu "Control Tab" skipped the cloaked/internal-page guard (**privacy leak**, rule 7), content-script inject, and stop-recording-on-release. | **Fixed** — shared `src/lib/tab-tracking-core.ts` `trackTab`/`untrackTab`; both entry points route through it. Tests: `tab-tracking-core`, `entry-point-parity`. |
