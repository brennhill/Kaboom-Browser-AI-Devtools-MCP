...
| `ARCH_INV_006` | Extension/server sync protocol changes are coordinated in one change set. | `internal/capture/syncruntime/handler.go`, `internal/capture/httpingest/handlers.go`, `src/background/sync/sync-client.ts` | `go test ./internal/capture/...` and extension test target |
| `ARCH_INV_007` | Sensitive data handling preserves redaction guarantees in logs and surfaced outputs. | `internal/redaction/`, `cmd/browser-agent/internal/toolobserve/dispatcher.go`, export paths | `go test ./internal/redaction/...` plus relevant tool tests |
