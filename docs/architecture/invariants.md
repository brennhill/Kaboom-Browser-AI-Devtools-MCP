...
| `ARCH_INV_006` | Extension/server sync protocol changes are coordinated in one change set. | `internal/capture/sync.go`, `internal/capture/httpingest/handlers.go`, `src/background/sync/sync-client.ts` | `go test ./internal/capture/...` and extension test target |
