...
11. Installer reveals the exact extension folder (`openExtensionFolder` / `openExtensionDir`) and opens the detected Chromium-family browser straight to its extensions page (`detectExtensionsTarget` → `openExtensionsPage`, browser-internal schemes `chrome://`/`brave://`/`edge://`). `detectExtensionsTarget` prefers the **OS default browser** (`detectDefaultBrowserId`: macOS LaunchServices, Linux `xdg-settings`, Windows `UserChoice` ProgId) when it is a supported, installed Chromium browser — otherwise it falls back to the fixed preference order.
...
11. The connect loop's clock, `/health` fetch, and output sink are injectable so the loop is deterministic under test (no real timers or daemon). The real HTTP shells (`defaultRequest` in JS, `fetchInstallHealth` in Go) are covered against a local test server. The connect wait also accepts an abort (JS `AbortSignal`, Go `context.Context`), checked at the loop top so a skip is honored within one poll cycle.
12. The CLI integration harness (`tests/cli/cli-integration.test.cjs`) sandboxes `HOME`/`USERPROFILE` to a seeded temp dir so real `--install` runs never mutate the developer's/CI runner's actual MCP client configs.
12. Dev-binary resolution has a single owner, `lib/runtime/resolve-binary.js` (used by `resolveManagedBinaryPath` and mirrored by the `bin/` shell shims). It uses the repo-root dist name `kaboom-agentic-browser-<platformKey>` — matching the Makefile's `$(BINARY_NAME)-<platformKey>` output, not the former `kaboom-<platformKey>` (which never matched). The dist candidate is added **only in the source tree** (package parent dir is `npm`, never `node_modules`), so an installed package can never resolve/exec a `dist/` planted in a user's project. `KABOOM_BINARY_PATH` (and `KABOOM_HOOKS_BINARY_PATH` for hooks) overrides everything, and a set-but-missing override fails rather than falling through. PATH is never consulted: the launchers no longer probe `command -v`/`where`, and resolution failure raises `BinaryNotFoundError` naming the missing optionalDependency instead of returning a bare command name for the client to resolve at spawn time.
