---
status: shipped
scope: feature/pr-preview-exploration/qa
ai-priority: medium
tags: [testing, qa]
...
| DL-3 | PR comment report exposes internal error messages | Verify that the PR summary report generated by `generate({what: "pr_summary"})` does not include internal server error details, database connection strings, or stack traces from the preview environment | high |
...
| DL-5 | SARIF output contains internal infrastructure paths | Verify that `generate({what: "sarif"})` does not expose internal file paths, server infrastructure details, or internal API endpoint structures from the preview environment | high |
| DL-6 | Baseline snapshot contains production credentials | Verify that when the agent captures a live baseline from the production/stable-branch site, no production credentials, session tokens, or API keys are captured in the snapshot | critical |
...
| CL-11 | Tab isolation confirmation | Verify the AI receives the `tab_id` from `interact({what: "new_tab"})` and understands subsequent interactions must target this tab, not the developer's tabs | [ ] |
...
|---|-----------|-------|-----------------|----------|
| UT-1 | Navigate to preview via new_tab | `interact({what: "new_tab", url: "https://preview-42.myapp.dev"})` | New tab opened, tab_id returned | must |
| UT-2 | Session capture for baseline | `configure({what: "diff_sessions", session_action: "capture", name: "stable-baseline"})` | Named snapshot saved in session manager | must |
| UT-3 | Session capture for preview | `configure({what: "diff_sessions", session_action: "capture", name: "preview-pr-42"})` | Named snapshot saved with preview telemetry | must |
| UT-4 | Session diff (baseline vs preview) | `configure({what: "diff_sessions", session_action: "compare", compare_a: "stable-baseline", compare_b: "preview-pr-42"})` | Structured diff showing new errors, changed network responses, performance deltas | must |
...
| UT-8 | PR summary generation | `generate({what: "pr_summary"})` | Markdown report with errors, performance, and findings | must |
| UT-9 | SARIF generation | `generate({what: "sarif"})` | Valid SARIF JSON with findings | could |
| UT-10 | URL convention template retrieval | `configure({what: "store", store_action: "load", key: "preview_url_template"})` | Stored template returned (e.g., `https://preview-{pr_number}.myapp.dev`) | should |
...
| UT-12 | Behavioral baseline comparison | `configure({what: "compare_baseline", name: "stable-baseline"})` | Regression categories: network, timing, console, WebSocket | should |
| UT-13 | execute_js for page interaction | `interact({what: "execute_js", script: "document.querySelector('a[href=\"/dashboard\"]').click()"})` | Navigation triggered, page changes | must |
...
|---|-----------|--------|--------|----------|
| PT-1 | Navigate to preview overhead | Kaboom overhead for `interact({what: "navigate"})` | < 2s (Kaboom overhead, actual page load depends on preview) | must |
...
| PT-5 | PR summary generation | `generate({what: "pr_summary"})` | < 300ms | must |
...
| UAT-2 | **Phase 2 - Baseline:** AI navigates to stable branch: `{"tool":"interact","arguments":{"what":"navigate","url":"http://localhost:3000"}}` | Browser navigates to stable app | Stable branch app loads | [ ] |
| UAT-3 | AI captures baseline: `{"tool":"configure","arguments":{"what":"diff_sessions","verif_session_action":"capture","name":"stable-baseline"}}` | N/A | Baseline snapshot captured and saved | [ ] |
| UAT-4 | AI checks baseline for errors: `{"tool":"observe","arguments":{"what":"errors"}}` | No errors in stable branch | Error list empty or only known pre-existing errors | [ ] |
| UAT-5 | **Phase 3 - Explore:** AI opens preview in new tab: `{"tool":"interact","arguments":{"what":"new_tab","url":"http://localhost:3001"}}` | New browser tab opens with preview app | Tab opened, tab_id returned, developer's tab unaffected | [ ] |
| UAT-6 | AI waits for page load: `{"tool":"observe","arguments":{"what":"page"}}` | Preview page loads in new tab | Page ready state reported | [ ] |
| UAT-7 | AI explores -- clicks navigation link: `{"tool":"interact","arguments":{"what":"execute_js","script":"document.querySelector('a[href=\"/dashboard\"]').click()"}}` | Browser navigates to dashboard in preview tab | Dashboard page loads (with intentional console error) | [ ] |
...
| UAT-10 | AI captures preview snapshot: `{"tool":"configure","arguments":{"what":"diff_sessions","verif_session_action":"capture","name":"preview-pr-test"}}` | N/A | Preview session snapshot captured | [ ] |
| UAT-11 | **Phase 4 - Compare:** AI compares sessions: `{"tool":"configure","arguments":{"what":"diff_sessions","verif_session_action":"compare","compare_a":"stable-baseline","compare_b":"preview-pr-test"}}` | N/A | Diff shows new error in preview not present in baseline, performance delta | [ ] |
| UAT-12 | AI checks accessibility: `{"tool":"observe","arguments":{"what":"accessibility"}}` | N/A | Accessibility findings returned (if any) | [ ] |
| UAT-13 | **Phase 5 - Report:** AI generates report: `{"tool":"generate","arguments":{"what":"pr_summary"}}` | N/A | Markdown report with preview URL, findings, regressions, performance comparison | [ ] |
...
### Regression Checks
- [ ] Existing `interact({what: "navigate"})` works independently of preview exploration workflow
- [ ] Existing `interact({what: "new_tab"})` works independently
- [ ] Existing `interact({what: "execute_js"})` works independently
- [ ] Existing `observe` tools (errors, network, performance, accessibility, page) work independently
- [ ] Existing `configure({what: "diff_sessions"})` works independently
- [ ] Existing `generate({what: "pr_summary"})` works independently
