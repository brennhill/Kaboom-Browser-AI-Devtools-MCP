...
| UT-2 | Session capture for baseline | `configure({action: "diff_sessions", session_action: "capture", name: "stable-baseline"})` | Named snapshot saved in session manager | must |
...
| UT-4 | Session diff (baseline vs preview) | `configure({action: "diff_sessions", session_action: "compare", compare_a: "stable-baseline", compare_b: "preview-pr-42"})` | Structured diff showing new errors, changed network responses, performance deltas | must |
...
| UT-12 | Behavioral baseline comparison | `configure({action: "compare_baseline", name: "stable-baseline"})` | Regression categories: network, timing, console, WebSocket | should |
...
| UAT-2 | **Phase 2 - Baseline:** AI navigates to stable branch: `{"tool":"interact","arguments":{"action":"navigate","url":"http://localhost:3000"}}` | Browser navigates to stable app | Stable branch app loads | [ ] |
| UAT-3 | AI captures baseline: `{"tool":"configure","arguments":{"action":"diff_sessions","verif_session_action":"capture","name":"stable-baseline"}}` | N/A | Baseline snapshot captured and saved | [ ] |
...
| UAT-11 | **Phase 4 - Compare:** AI compares sessions: `{"tool":"configure","arguments":{"action":"diff_sessions","verif_session_action":"compare","compare_a":"stable-baseline","compare_b":"preview-pr-test"}}` | N/A | Diff shows new error in preview not present in baseline, performance delta | [ ] |
