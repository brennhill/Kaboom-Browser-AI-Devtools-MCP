...
code_paths:
  ...
  - internal/instancereg/admission.go
  - internal/instancereg/cpu.go
...
test_paths:
  ...
  - internal/instancereg/admission_test.go
...
4. **Admission with caps** (`instancereg.Decide`). One production daemon per
   machine; test daemons capped at `min(4, max(2, cores/4))`, evicting oldest-first.
