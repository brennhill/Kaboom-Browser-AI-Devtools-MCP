...
- [ ] Webhook handler must not forward GitHub webhook secrets to Kaboom
...
- [ ] Self-healing: Skill `preconditions` automatically check Kaboom health before attempting diagnosis
...
| UT-2 | Skill precondition health check fails | `GET /health` returns 503 or connection refused | Skill returns clear error: "Kaboom server not running" | must |
...
| EC-4 | Kaboom server restarts during workflow | Server restarts after baseline capture but before comparison | Agent detects server unavailability, re-captures baseline, retries workflow | should |
| EC-5 | No browser tab available | CI environment has no headed browser | Agent uses headless capture via kaboom-ci.js, not extension | must |
...
|---|-------------------|----------------|-----------------|------|
| UAT-1 | AI checks Kaboom health: `GET http://127.0.0.1:7890/health` | Server responds in terminal | HTTP 200 returned, AI confirms server is running | [ ] |
