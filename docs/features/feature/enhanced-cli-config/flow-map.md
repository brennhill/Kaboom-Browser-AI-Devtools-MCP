...
- Dev-binary resolution fix: the launcher (`findBinary`) and `resolveManagedBinaryPath` now look for the repo-root dist name `kaboom-agentic-browser-<platformKey>` (matching the Makefile), source-tree-gated (package parent `npm`, never `node_modules`) so an installed package can't exec a `dist/` in a user's project. `resolveManagedBinaryPath` is now dependency-injectable and unit-tested.
- Doctor's port check is net-based (`checkPort`) and cross-platform — the dead `lsof`-based `checkPortSync` was removed. The real `/health` HTTP shells (`defaultRequest`, `fetchInstallHealth`) are now covered against a local test server.
