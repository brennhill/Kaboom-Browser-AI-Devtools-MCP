...
function enrichEntry(
  entry: WireNetworkWaterfallEntry,
...
function cleanStack(raw: string | undefined): string[] {
  if (!raw) return []
  return raw
    .split('\n')
    .map((line) => line.trim())
...
function semanticInitiator(stack: string[]): Partial<WireNetworkWaterfallEntry> {
  ...
  let routeLoader: string | undefined
  let storeAction: string | undefined
  for (const frame of stack) {
    const name = /^at\s+([^\s(]+)/.exec(frame)?.[1]
    if (!name) continue
    if (!reactComponent && /^[A-Z][A-Za-z0-9_$]*$/.test(name)) reactComponent = name
    if (!routeLoader && /(?:loader|route)/i.test(name)) routeLoader = name
    if (!storeAction && /(?:store|dispatch|action|mutation)/i.test(name)) storeAction = name
  }
  return {
...
function parseServerTiming(raw: string): NonNullable<WireNetworkWaterfallEntry['server_timing']> {
  return raw
    .split(',')
    .slice(0, 20)
    .map((metric) => {
      const [namePart = '', ...params] = metric.trim().split(';')
      const result: { name: string; duration_ms?: number; description?: string } = { name: namePart.slice(0, 128) }
      for (const param of params) {
        const [key, rawValue = ''] = param.trim().split('=', 2)
        const value = rawValue.replace(/^"|"$/g, '')
        if (key === 'dur' && Number.isFinite(Number(value))) result.duration_ms = Number(value)
        if (key === 'desc' && value) result.description = value.slice(0, 256)
      }
      return result
    })
...
function assignDuplicateGroups(entries: WireNetworkWaterfallEntry[]): void {
  const byURL = new Map<string, WireNetworkWaterfallEntry[]>()
  ...
  for (const [url, group] of byURL) {
    if (group.length < 2 || !hasOverlap(group)) continue
...
function hasOverlap(entries: WireNetworkWaterfallEntry[]): boolean {
  const sorted = [...entries].sort((a, b) => a.start_time - b.start_time)
  let latestEnd = -Infinity
  for (const entry of sorted) {
    if (entry.start_time < latestEnd) return true
