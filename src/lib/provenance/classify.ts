 ...
 */
export function classifyRegion(facts: RegionFacts): ProvenanceClassification {
  if (facts.delivered_in_initial_document === false) return 'post_load_injected'
  if (!sameOrigin(facts.origin, facts.document_origin)) return 'third_party_frame'
  if (facts.is_frame || !facts.is_top_level_document) return 'same_origin_subresource'
...
export function countByClassification(
  regions: readonly Pick<ProvenanceRegion, 'classification'>[]
): Record<ProvenanceClassification, number> {
  const counts = {
  ...
  for (const region of regions) {
    if (PROVENANCE_CLASSIFICATIONS.includes(region.classification)) counts[region.classification] += 1
  }
  return counts
}
...
export function frameProvenance(documentOrigin: string, regions: ProvenanceRegion[]): ContentProvenance {
  return {
 ...
 */
export function unavailableProvenance(reason: string, extraNotes: readonly string[] = []): ContentProvenance {
  return {
