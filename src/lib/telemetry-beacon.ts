...
import {
  KABOOM_TELEMETRY_ENDPOINT,
  KABOOM_TELEMETRY_STORAGE_KEY
} from './brand.js'

// Opt-out flag cached from chrome.storage.local for synchronous checks.
let telemetryDisabled = false

// Resolves once the opt-out flag has been hydrated from storage. beacon() awaits
// this so events fired during startup (e.g. extension_start) respect the opt-out.
let telemetryFlagHydrated: Promise<void> = Promise.resolve()

// Check opt-out on load
try {
  telemetryFlagHydrated = new Promise<void>((resolve) => {
    try {
...
export function beacon(event: string, props: Record<string, string> = {}): void {
  telemetryFlagHydrated
    .then(() => {
      sendBeaconNow(event, props)
    })
    .catch(() => {
      /* never throw */
    })
...
function sendBeaconNow(event: string, props: Record<string, string>): void {
