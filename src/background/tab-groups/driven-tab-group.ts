...
export const DRIVEN_TAB_GROUP_COLOR = 'purple' as const
...
/** The group Kaboom currently owns, keyed to the daemon connection generation that opened it. */
...
let session: DrivenSession | null = null
let permissionRequested = false
let reconciledThisWorker = false
let unsubscribeConnection: (() => void) | null = null
let lastDegradeReason: string | null = null
 ...
 */
function degrade(reason: string, detail?: Record<string, unknown>): DrivenTabGroupOutcome {
...
function tabGroupApisPresent(): boolean {
  return (
...
async function holdsTabGroupsPermission(): Promise<boolean> {
  if (typeof chrome?.permissions?.contains !== 'function') return tabGroupApisPresent()
  try {
 ...
 */
async function groupingBlockedBy(): Promise<DrivenTabGroupOutcome | null> {
  if (!tabGroupApisPresent()) return degrade('tab_groups_api_unavailable')
  if (await holdsTabGroupsPermission()) {
    clearDegrade()
    return null
  }
  if (permissionRequested) return degrade('permission_denied')
  permissionRequested = true
  if (typeof chrome?.permissions?.request !== 'function') return degrade('permission_request_unavailable')
  ...
  if (!granted) return degrade('permission_denied')
  clearDegrade()
  return null
}
...
async function liveGroupExists(groupId: number): Promise<boolean> {
...
async function releaseGroup(groupId: number, reason: string): Promise<number> {
 ...
 */
export async function reconcileDrivenTabGroups(): Promise<number> {
  if (typeof chrome?.tabGroups?.query !== 'function') return 0
  ...
  for (const group of groups) {
    if (session !== null && group.id === session.groupId) continue
...
export async function endDrivenTabGroupSession(reason: string): Promise<void> {
...
function installSessionHooks(): void {
  ...
  unsubscribeConnection = subscribeExtensionConnection((connected) => {
    if (connected) return
    endDrivenTabGroupSession('daemon_disconnected').catch((err: unknown) => {
...
async function reconcileOncePerWorker(): Promise<void> {
  if (reconciledThisWorker) return
 ...
 */
async function rotateOnSessionChange(): Promise<void> {
...
async function dropSessionIfGroupGone(): Promise<void> {
  if (session === null) return
  if (await liveGroupExists(session.groupId)) return
...
async function labelDrivenGroup(groupId: number): Promise<void> {
  try {
...
async function joinSessionGroup(tabId: number, entryPoint: DrivenTabGroupEntryPoint): Promise<DrivenTabGroupOutcome> {
  ...
  try {
    ...
    if (existing === null || existing.groupId !== groupId) await labelDrivenGroup(groupId)
 ...
 */
export async function adoptTabIntoDrivenGroup(
  tabId: number,
  entryPoint: DrivenTabGroupEntryPoint
): Promise<DrivenTabGroupOutcome> {
  if (!Number.isInteger(tabId) || tabId < 0) return { adopted: false, degraded_reason: 'invalid_tab_id' }
 ...
 */
export async function noteDrivenTabClosed(tabId: number): Promise<void> {
  if (session === null) return
  const groupId = session.groupId
  if (await liveGroupExists(groupId)) return
  session = null
...
export function getDrivenTabGroupId(): number | null {
  return session === null ? null : session.groupId
}

/** Drop all in-memory ownership so each test starts from a cold worker. */
export function resetDrivenTabGroupStateForTesting(): void {
  if (unsubscribeConnection !== null) unsubscribeConnection()
