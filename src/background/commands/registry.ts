...
): Promise<{ target: TargetResolution | undefined; handled: boolean }> {
  try {
...
async function awaitContentReadinessIfNeeded(
  queryType: string,
  tabId: number,
  lifecycle: DispatchLifecycle
): Promise<boolean> {
  if (!requiresContentReadiness(queryType) || !contentReadiness.hasPending(tabId)) return true
...
export async function dispatch(query: PendingQuery, syncClient: SyncClient, signal: AbortSignal): Promise<void> {
  ...
  if (!(await awaitContentReadinessIfNeeded(query.type, tabId, lifecycle))) return
