 ...
 */
function contentExtractorCommand(messageType: string, errorCode: string): CommandHandler {
  return async (ctx) => {
    try {
      ...
      if (isContentScriptUnreachableError(err)) {
        ...
        if (fallbackFn) {
          try {
            ...
            ctx.sendResult({
              error: errorCode,
              message: `Content script not loaded and fallback injection failed: ${(fallbackErr as Error).message || 'unknown error'}. Refresh the page first: interact({what: "refresh"}), then retry.`
            })
            return
          }
        }
