...
// driving-session.ts — Per-tab driving state, heartbeat, and the user stop path.
...
export interface DrivingSessionDeps {
  ...
  notify: (tabId: number, phase: IndicatorPhase, detail?: { action?: string; x?: number; y?: number }) => void
  /**
...
export class DrivingSessions {
  ...
  private readonly deps: Required<DrivingSessionDeps>
  ...
  private readonly stopRequests = new Set<number>()
...
let shared: DrivingSessions | null = null

/** The one driving-session registry. All CDP input paths must route through it. */
export function drivingSessions(): DrivingSessions {
 ...
 * different tab.
 */
export function createSupervisionMessageHandler(
  sessions: DrivingSessions = drivingSessions()
): MessageHandlerOwner {
  return {
    feature: 'agent-supervision',
    handle(message: BackgroundMessage, sender: ChromeMessageSender, sendResponse: SendResponse) {
      if ((message as { type?: string }).type !== 'kaboom_agent_stop_requested') return undefined
