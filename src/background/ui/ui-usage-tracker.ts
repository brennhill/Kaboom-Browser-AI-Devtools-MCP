...
import { SYNC_UI_FEATURES, type SyncFeaturesUsed, type SyncUIFeature } from '../../types/wire/wire-sync.js'
...
const validFeatures = new Set<string>(SYNC_UI_FEATURES)
let pending: Map<SyncUIFeature, boolean> = new Map()
 ...
 */
export function trackUIFeature(feature: SyncUIFeature): void {
 ...
 */
export function drainUIFeatures(): SyncFeaturesUsed | undefined {
  if (pending.size === 0) return undefined
  const old = pending
  pending = new Map()
  const result: Partial<Record<SyncUIFeature, boolean>> = {}
  for (const [key, val] of old) {
    result[key] = val
  }
  return result as SyncFeaturesUsed
}
 ...
 */
export function restoreUIFeatures(features: SyncFeaturesUsed | Readonly<Record<string, boolean>>): void {
  let rejectedCount = 0
  for (const [key, val] of Object.entries(features)) {
    ...
    pending.set(key as SyncUIFeature, true)
  }
  if (rejectedCount > 0) {
