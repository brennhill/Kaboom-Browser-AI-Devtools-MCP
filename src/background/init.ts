 ...
 */
/**
 * The one local the message handlers close over. Everything else they need is a module
 * import, so the context stays a single value rather than a parameter list.
 */
interface MessageHandlerContext {
  setPilotEnabled: (enabled: boolean, callback?: () => void) => void
}
 ...
 */
function installBackgroundMessageHandlers(ctx: MessageHandlerContext): void {
...
async function initializeExtensionAsync(): Promise<void> {
  try {
    ...
    trackingContinuity.subscribe((snapshot) => {
      void chrome.runtime.sendMessage({ type: 'tracking_continuity_changed', snapshot }).catch((error: unknown) => {
