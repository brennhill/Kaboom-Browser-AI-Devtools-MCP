...
export class SyncClient {
  ...
  updateCommandProgress(commandId: string, progressPct?: number, status: 'running' | 'pending' = 'running'): void {
    ...
    if (!current) return
    const progress =
      typeof progressPct === 'number' && Number.isFinite(progressPct) ? { progress_pct: clampPercent(progressPct) } : {}
  ...
  private async doSync(): Promise<void> {
    ...
    let features: SyncFeaturesUsed | undefined
    ...
    try {
      ...
      if (this.rejectStaleSync(lifecycleEpoch)) return
  ...
  private buildSyncRequest(
    ...
    logs: ExtensionLog[],
    resultsSentCount: number,
    features: SyncFeaturesUsed | undefined
  ): SyncRequest {
    return {
  ...
  private async fetchSyncResponse(request: SyncRequest): Promise<Response> {
  ...
  private rejectStaleSync(lifecycleEpoch: number): boolean {
  ...
  private notifyVersionMismatch(data: SyncResponse): void {
    // Check for version mismatch (compare major.minor only, ignore patch)
    if (!data.server_version || !this.extensionVersion || !this.callbacks.onVersionMismatch) return
  ...
  private dispatchSyncCommands(data: SyncResponse): void {
  ...
  private scheduleNextPoll(data: SyncResponse): void {
    this.syncing = false
  ...
  private handleSyncFailure(err: unknown, features: SyncFeaturesUsed | undefined): void {
    this.syncing = false
    this.flushRequested = false
    this.onFailure()
