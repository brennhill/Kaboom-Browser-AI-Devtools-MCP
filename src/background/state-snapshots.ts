...
function readSnapshots(): Promise<StateSnapshotStorage> {
  return readLocalState<StateSnapshotStorage>({
    key: SNAPSHOT_KEY,
    fallback: {},
    validate: (value): value is StateSnapshotStorage =>
      typeof value === 'object' &&
      value !== null &&
      !Array.isArray(value) &&
      Object.values(value).every(
        (snapshot) =>
          ...
          snapshot !== null &&
          typeof (snapshot as StoredStateSnapshot).name === 'string' &&
          typeof (snapshot as StoredStateSnapshot).size_bytes === 'number'
      ),
    diagnostic: {
