...
// cdp-env-pin.ts — Opt-in per-session environment pinning over the shared CDP lease.
 ...
 */
export function seedInstallSnippet(seed: string): string {
...
/** One pinned knob and the CDP call that pins it. */
interface PinStep {
  readonly name: string
  readonly wanted: boolean
  run(lease: Lease): Promise<void>
}

function clockSteps(spec: EnvironmentPinSpec): PinStep[] {
  ...
  return [
    ...
    },
    {
      ...
      run: (lease) =>
        lease.send('Emulation.setTimezoneOverride', { timezoneId: spec.timezone_id }).then(() => undefined)
    }
  ]
}

function placeSteps(spec: EnvironmentPinSpec): PinStep[] {
  return [
    {
      name: 'geolocation',
 ...
 */
async function runSeedStep(lease: Lease, seed: string): Promise<void> {
...
function seedStep(spec: EnvironmentPinSpec): PinStep {
  return {
    name: 'random seed',
...
function pinSteps(spec: EnvironmentPinSpec): PinStep[] {
 ...
 */
function clockReport(spec: EnvironmentPinSpec, landed: Set<string>): WireEnvironmentPin['clock'] {
  const clock = landed.has('clock')
  const timezone = landed.has('timezone')
  if (!clock && !timezone) return undefined
  return {
...
function geoReport(spec: EnvironmentPinSpec, landed: Set<string>): WireEnvironmentPin['geolocation'] {
  if (!landed.has('geolocation')) return undefined
  return { latitude: spec.latitude ?? 0, longitude: spec.longitude ?? 0, accuracy_m: spec.accuracy_m ?? 1 }
}

function viewportReport(spec: EnvironmentPinSpec, landed: Set<string>): WireEnvironmentPin['viewport'] {
  if (!landed.has('viewport')) return undefined
  return {
...
function buildPinReport(spec: EnvironmentPinSpec, landed: Set<string>, unpinned: string[]): WireEnvironmentPin {
 ...
 */
export async function applyEnvironmentPin(lease: Lease, spec: EnvironmentPinSpec): Promise<WireEnvironmentPin> {
 ...
 */
export async function clearEnvironmentPin(lease: Lease): Promise<string[]> {
...
const pinnedTabs = new Map<number, WireEnvironmentPin>()

export function recordEnvironmentPin(tabId: number, pin: WireEnvironmentPin): void {
  pinnedTabs.set(tabId, pin)
}

export function environmentPinFor(tabId: number | undefined): WireEnvironmentPin | undefined {
  if (typeof tabId !== 'number') return undefined
  return pinnedTabs.get(tabId)
}

export function forgetEnvironmentPin(tabId: number): void {
