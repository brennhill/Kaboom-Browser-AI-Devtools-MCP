 ...
 * flag.
 */
export function addressedPoints(params: CoordinateParams): AddressedPoint[] {
  const points: AddressedPoint[] = []
 ...
 */
export function outOfViewportMessage(
  action: string,
  points: readonly AddressedPoint[],
  extent: ViewportExtent | null
): string | null {
  if (!extent || !(extent.width > 0) || !(extent.height > 0)) return null
  for (const point of points) {
    if (onScreen(point, extent)) continue
    return (
      `${action} was aimed at (${point.x}, ${point.y}), which is outside the viewport — ` +
...
function onScreen(point: AddressedPoint, extent: ViewportExtent): boolean {
 ...
 */
export async function readViewportExtent(tabId: number): Promise<ViewportExtent | null> {
  try {
 ...
 */
export async function coordinateOutOfViewport(
  tabId: number,
  action: string,
