...
export function domPrimitiveActionDiff(options?: { timeout_ms?: number }): Promise<{
  ...
  action_diff?: {
    ...
    overlays_opened: Array<{ selector: string; text: string }>
    overlays_closed: Array<{ selector: string; text: string }>
    toasts: Array<{ text: string; type: string }>
    form_errors: string[]
    loading_indicators: string[]
    elements_added: number
    elements_removed: number
    text_changes: Array<{ selector: string; from: string; to: string }>
