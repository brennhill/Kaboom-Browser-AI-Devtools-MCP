...
// dom-primitives-gestures.ts — Injected gesture dispatchers plus their service-worker helpers.
//
// The three exported `domGesture*` functions are injected via chrome.scripting.executeScript({func}),
// which serializes the function source WITHOUT module scope: every helper they use must live
// inside them, and nothing may close over an import. Two consequences shape this file:
//   - they take an already resolved COORDINATE, not a selector, so no copy of the selector
//     engine lives in page scope (resolveElement does that lookup once in the service worker);
//   - they take an already folded modifier BITMASK, not modifier names, so the name-to-bit
//     table exists exactly once and both dispatch paths report the same number.
...
const GESTURE_MODIFIER_BITS: Record<string, number> = {
  alt: 1,
  ctrl: 2,
  control: 2,
  meta: 4,
...
export const DOM_GESTURE_ACTIONS: ReadonlySet<string> = new Set([
  'drag',
  'right_click',
  'double_click',
  'triple_click',
...
export function isDOMGestureAction(action: string): boolean {
 ...
 */
export function needsGestureDispatch(action: string, modifiers: string[] | undefined): boolean {
  if (isDOMGestureAction(action)) return true
  return action === 'click' && Array.isArray(modifiers) && modifiers.length > 0
}

export function gestureModifierMask(modifiers: readonly string[] | undefined): number {
 ...
 */
export function domGestureClick(action: string, at: GesturePagePoint, options: GesturePageOptions): GesturePageOutcome {
  const node = document.elementFromPoint(at.x, at.y)
  if (!node) return { dispatched: false, error: 'no_element_at_point' }
  const mask = options.modifiers

  function fire(type: string, extra: Record<string, unknown>): void {
    node!.dispatchEvent(
      new MouseEvent(type, {
        bubbles: true,
        cancelable: true,
  ...
  function cycle(button: number, detail: number): void {
  ...
  if (!total) return { dispatched: false, error: 'unknown_action' }
  for (let detail = 1; detail <= total; detail += 1) cycle(0, detail)
  if (total === 2) fire('dblclick', { button: 0, buttons: 0, detail: 2 })
 ...
 * MUST NOT reference module-level values — Chrome serializes the function source only.
 */
export function domGestureScroll(action: string, at: GesturePagePoint, options: GesturePageOptions): GesturePageOutcome {
  const node = document.elementFromPoint(at.x, at.y)
  if (!node) return { dispatched: false, error: 'no_element_at_point' }
  const mask = options.modifiers

  function base(extra: Record<string, unknown>): MouseEventInit {
    return {
      bubbles: true,
      cancelable: true,
      composed: true,
  ...
  function scrollable(): Element {
    ...
    while (box && box !== document.documentElement) {
      ...
      if (scrolls && /auto|scroll/.test(overflow)) return box
  ...
  if (action !== 'scroll_at') return { dispatched: false, error: 'unknown_action' }
 ...
 * MUST NOT reference module-level values — Chrome serializes the function source only.
 */
export function domGestureDrag(_action: string, at: GesturePagePoint, options: GesturePageOptions): GesturePageOutcome {
  const route = Array.isArray(options.drag_path) ? options.drag_path : []
  if (route.length < 2) return { dispatched: false, error: 'invalid_drag_path' }
  const mask = options.modifiers

  /** A press-then-jump-then-release moves nothing: drag libraries start on the first move. */
  function densify(): GesturePagePoint[] {
    const dense: GesturePagePoint[] = [route[0]!]
  ...
  function base(p: GesturePagePoint, extra: Record<string, unknown>): MouseEventInit {
    return {
      bubbles: true,
      cancelable: true,
      composed: true,
  ...
  function drag(node: Element, type: string, p: GesturePagePoint): void {
...
export function gesturePrimitiveFor(action: string): typeof domGestureClick {
  if (action === 'drag') return domGestureDrag
  if (action === 'hover_at' || action === 'scroll_at') return domGestureScroll
 ...
 */
export function buildGestureDOMResult(
  action: string,
  selector: string,
  point: GesturePagePoint,
  resolved: ResolvedElement | null,
  outcome: GesturePageOutcome | undefined,
  modifiers: number
): DOMResult {
...
function gestureMatchedEvidence(resolved: ResolvedElement): Record<string, unknown> {
  return {
    tag: resolved.tag,
