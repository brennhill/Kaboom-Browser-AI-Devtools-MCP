...
trim();if(t&&t.length>0){sections.push({content:t,tag})}}}return sections}function buildActionHandlers(node){return{get_text:()=>{if(options.structured&&node instanceof
HTMLElement){const sections=structuredTextSections(node);return{success:true,action,selector,sections,section_count:sections.length}}const text=node instanceof HTMLElement?
node.innerText:node.textContent;if(text===null||text===void 0){return{success:true,action,selector,value:text,reason:"no_text_content",message:"Resolved text co\
ntent is null"}}return{success:true,action,selector,value:text}},get_value:()=>{if(!("value"in node))return domError("no_value_property",`Element has no value p\
roperty: ${node.tagName}`);const value=node.value;if(value===null||value===void 0){return{success:true,action,selector,value,reason:"no_value",message:"Element \
value is null"}}return{success:true,action,selector,value}},get_attribute:()=>{const attrName=options.name||"";const value=node.getAttribute(attrName);if(value===
null){return{success:true,action,selector,value,reason:"attribute_not_found",message:`Attribute "${attrName}" not found`}}return{success:true,action,selector,value}},
wait_for:()=>({success:true,action,selector,value:node.tagName.toLowerCase()}),wait_for_text:()=>{const searchText=options.text||"";if(!searchText){return{success:false,
action,selector:"",error:"empty_text",message:"text parameter is required for wait_for_text"}}const bodyText=document.body?.innerText??"";if(bodyText.includes(searchText)){
return{success:true,action,selector:"",matched_text:searchText}}return{success:false,action,selector:"",error:"text_not_found"}},wait_for_absent:()=>{if(!selector){
return{success:false,action,selector:"",error:"missing_selector",message:"selector is required for wait_for_absent"}}const el2=resolveElement(selector);if(!el2){
return{success:true,action,selector,absent:true}}return{success:false,action,selector,error:"element_still_present"}}}}const handlers=buildActionHandlers(el);const handler=handlers[action];
