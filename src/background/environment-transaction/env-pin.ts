...
import {
  ...
  clearEnvironmentPin,
  environmentPinFor,
  forgetEnvironmentPin,
  recordEnvironmentPin,
  type EnvironmentPinSpec
} from '../dom/cdp/cdp-env-pin.js'
 ...
 */
export function parseEnvironmentPinSpec(params: Readonly<Record<string, unknown>>): EnvironmentPinSpec {
  const source = (params.environment && typeof params.environment === 'object' ? params.environment : params) as Record<
    string,
    unknown
  >
  ...
  for (const key of NUMERIC_KEYS) {
    ...
    if (typeof value === 'number' && Number.isFinite(value)) spec[key] = value
  }
  if (typeof source.timezone_id === 'string' && source.timezone_id) spec.timezone_id = source.timezone_id
  if (typeof source.random_seed === 'string' && source.random_seed) spec.random_seed = source.random_seed
  if (source.mobile === true) spec.mobile = true
  ...
  return spec as EnvironmentPinSpec
}

/** True when the caller asked for nothing at all — an empty pin is a caller error, not a no-op. */
function isEmptySpec(spec: EnvironmentPinSpec): boolean {
  return Object.keys(spec).length === 0
}

export function registerEnvironmentPinCommands(): void {
  registerCommand('env_pin', async (ctx) => {
    ...
    if (isEmptySpec(spec)) {
      ctx.sendResult({
        ...
        message:
          'pin_environment needs at least one knob, for example environment={"timezone_id":"UTC","random_seed":"run-1"}.'
      })
      return
    }
