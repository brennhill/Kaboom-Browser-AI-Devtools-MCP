...
function isValidOptionsState(candidate: StorageResult): boolean {
  return (
    isOptionalString(candidate.serverUrl) &&
...
export function saveOptions(): Promise<void> {
  const serverUrlInput = document.getElementById('server-url-input') as HTMLInputElement | null
 ...
 */
export function toggleDeferral(): void {
...
export async function testConnection(): Promise<void> {
  const btn = document.getElementById('test-connection-btn') as HTMLButtonElement | null
  ...
  try {
    ...
    const data = (await resp.json()) as { version?: string; logs?: { entries?: number } }
    if (resultEl) {
 ...
 */
export async function handleExportDebugLog(): Promise<ExportResult> {
  const exportBtn = document.getElementById('export-debug-btn') as HTMLButtonElement | null
