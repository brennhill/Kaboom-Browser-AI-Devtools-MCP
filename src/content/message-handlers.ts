...
type WaterfallResponseWaiter = (
  response: Promise<{ entries: WaterfallEntry[] }>,
  cleanup: () => void
) => Promise<{ entries: WaterfallEntry[] }>
...
export function handleGetNetworkWaterfall(
  sendResponse: (result: WaterfallBridgeResult) => void,
  waitForResponse: WaterfallResponseWaiter = waitForWaterfallResponse
): boolean {
  ...
  waitForResponse(deferred.promise, () => window.removeEventListener('message', responseHandler)).then(
    (result) => {
      sendResponse(result)
    },
    (error) => {
