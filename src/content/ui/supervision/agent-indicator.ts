...
export class AgentIndicatorCore {
  private state: AgentIndicatorState = {
    driving: false,
    action: null,
    cursor: null,
    lastHeartbeatAt: 0
  ...
  tick(): TeardownReason | null {
    ...
    if (this.now() - this.state.lastHeartbeatAt <= HEARTBEAT_TTL_MS) return null
    this.stopDriving()
 ...
 */
export function isHonouredStop(event: Pick<Event, 'isTrusted'> | null | undefined): boolean {
  return event?.isTrusted === true
}

/** Label shown in the pill. Kept here so wording and truncation stay consistent (rule 21). */
export function drivingLabel(action: string | null): string {
  const trimmed = (action ?? '').trim()
  if (!trimmed) return 'Kaboom is driving this tab'
...
export class AgentIndicator {
  ...
  private readonly core: AgentIndicatorCore
  ...
  mount(): void {
    if (this.rendered || typeof document === 'undefined' || !document.body) return
    ...
    stop.addEventListener('click', (event) => {
      ...
      if (!isHonouredStop(event)) return
      this.stopDriving()
      this.deps.onStop()
    })
  ...
  moveCursor(x: number, y: number): void {
    if (!this.core.moveCursor(x, y)) return
    this.paint()
  }
