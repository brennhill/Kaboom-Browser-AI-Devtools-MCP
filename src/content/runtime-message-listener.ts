...
function applyOverlayToggleState(result: Record<string, unknown>): void {
  ...
  if (
    (actionToasts !== undefined && typeof actionToasts !== 'boolean') ||
...
/** Phases the background sends to drive the supervision overlay. */
interface AgentIndicatorPhaseMessage {
 ...
 */
let agentIndicator: AgentIndicator | null = null
let agentIndicatorTimer: ReturnType<typeof setInterval> | null = null

/** How often the overlay checks its own liveness. */
const AGENT_INDICATOR_TICK_MS = 2_000

function ensureAgentIndicator(): AgentIndicator {
  if (agentIndicator) return agentIndicator
...
function stopAgentIndicatorTimer(): void {
  if (agentIndicatorTimer === null) return
  clearInterval(agentIndicatorTimer)
  agentIndicatorTimer = null
}

function handleAgentIndicatorMessage(msg: AgentIndicatorPhaseMessage): void {
    ...
    case 'driving':
      ...
      if (agentIndicatorTimer === null) {
        agentIndicatorTimer = setInterval(() => {
          if (indicator.tick() !== null) stopAgentIndicatorTimer()
        }, AGENT_INDICATOR_TICK_MS)
      }
      break
    case 'cursor':
      if (typeof msg.x === 'number' && typeof msg.y === 'number') indicator.moveCursor(msg.x, msg.y)
      break
    case 'heartbeat':
      indicator.heartbeat()
      break
...
function buildSyncHandlers(): Record<string, (msg: SyncMsg) => false | void> {
  return {
  kaboom_ping: () => {
  ...
  kaboom_agent_indicator: (msg) => {
    handleAgentIndicatorMessage(msg as AgentIndicatorPhaseMessage)
    return false
  },
...
function buildDelegatedHandlers(): Record<string, (msg: SyncMsg, sr: (r?: unknown) => void) => boolean | undefined> {
  return {
