...
import type { ProvenanceEnvironment } from './collect.js'
 ...
 */
function firstPartyOrigin(isTopLevel: boolean): string {
  if (isTopLevel) return toOrigin(window.location.href)
...
export function provenanceForExtraction(root: Element | null): ContentProvenance {
  const isTopLevel = window.top === window
  const env: ProvenanceEnvironment = {
