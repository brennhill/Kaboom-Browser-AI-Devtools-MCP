...
import type {
  ...
  ProvenanceAlert,
  ProvenanceRegion
} from '../../lib/provenance/provenance-types.js'
...
function frameText(frame: Element): string {
  try {
...
function deliveredInInitialDocument(node: Node | null, tracker: InjectionQuery): boolean | null {
...
function buildRegion(
  regionId: string,
  facts: { origin: string; is_frame: boolean; is_top_level_document: boolean; delivered: boolean | null },
...
function documentRegion(root: Element | null, env: ProvenanceEnvironment): ProvenanceRegion {
  ...
  return buildRegion(
    ...
    env,
    text,
    env.is_top_level_document ? null : env.document_origin
  )
}
 ...
 */
function frameRegions(root: Element | null, env: ProvenanceEnvironment): ProvenanceRegion[] {
  if (!root) return []
...
function injectedRegions(root: Element | null, env: ProvenanceEnvironment): ProvenanceRegion[] {
  if (!root) return []
  const inside = env.tracker.injectedRoots().filter((element) => root.contains(element))
  return inside.slice(0, MAX_INJECTED_REGIONS).map((element, index) =>
    buildRegion(
      `injected_${index + 1}`,
      { origin: env.frame_origin, is_frame: false, is_top_level_document: false, delivered: false },
      env,
      readText(element),
      nearestResourceOrigin(element, env)
    )
  )
}

/** The closest self-or-ancestor that names a URL — the only initiator the DOM alone can prove. */
function nearestResourceOrigin(element: Element, env: ProvenanceEnvironment): string | null {
 ...
 */
function alertsFor(regions: readonly ProvenanceRegion[]): ProvenanceAlert[] {
  const alerts: ProvenanceAlert[] = []
  for (const region of regions) {
    if (region.classification === 'first_party_document' || !region.imperative_text) continue
    alerts.push({
      ...
      message:
        `Text addressed to an agent appeared in a ${region.classification} region` +
...
function notesFor(env: ProvenanceEnvironment, regions: readonly ProvenanceRegion[], outside: number): string[] {
  const notes: string[] = []
  ...
  const firstPartyInjection = regions.filter(
    (region) => region.classification === 'post_load_injected' && sameOrigin(region.origin, env.document_origin)
  ).length
  if (firstPartyInjection > 0) {
    notes.push(
      `${firstPartyInjection} post-load injected region(s) are at the first-party origin, which is also how a ` +
 ...
 */
export function collectContentProvenance(root: Element | null, env: ProvenanceEnvironment): ContentProvenance {
