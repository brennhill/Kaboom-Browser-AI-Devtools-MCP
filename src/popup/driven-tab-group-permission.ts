...
function tabGroupsPermission(): chrome.permissions.Permissions {
  return { permissions: ['tabGroups'] }
}

function toggleElement(): HTMLInputElement | null {
  return (document.getElementById(DRIVEN_GROUP_TOGGLE_ID) as HTMLInputElement | null) ?? null
}
 ...
 * (CLAUDE.md rule 18).
 */
async function holdsPermission(): Promise<boolean> {
  if (typeof chrome?.permissions?.contains !== 'function') return false
  try {
    return await (chrome.permissions.contains(tabGroupsPermission()) as unknown as Promise<boolean>)
  } catch (err) {
...
async function reconcileToggle(toggle: HTMLInputElement): Promise<void> {
 ...
 */
function grant(toggle: HTMLInputElement): void {
  let requested: Promise<boolean>
  try {
    requested = chrome.permissions.request(tabGroupsPermission()) as unknown as Promise<boolean>
  } catch (err) {
...
function revoke(toggle: HTMLInputElement): void {
  const removal =
    typeof chrome?.permissions?.remove === 'function'
      ? (chrome.permissions.remove(tabGroupsPermission()) as unknown as Promise<boolean>)
 ...
 */
export function applyDrivenTabGroupToggle(): void {
  ...
  toggle.addEventListener('change', () => {
    if (toggle.checked) grant(toggle)
    else revoke(toggle)
  })
}
