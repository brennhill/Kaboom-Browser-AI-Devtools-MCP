...
describe('provenance never takes the content script down with it', () => {
  test('a document with no MutationObserver leaves the bootstrap running', async () => {
    // initContentProvenance() runs first in the content script's bootstrap. A throw here used to
    // abort the rest of it — message listeners, request tracking — leaving the tab undrivable with
    // no error anyone would connect to provenance. Reporting is never load-bearing.
    ...
    globalThis.document = { readyState: 'complete', documentElement: element('html') }
    globalThis.window = { addEventListener() {}, location: { href: FIRST_PARTY_HREF }, top: null }
    delete globalThis.MutationObserver

    try {
      const { initContentProvenance } = await import('../../../extension/content/provenance/index.js')
      initContentProvenance()
      assert.ok(
        warnings.some((line) => line.includes('provenance') && line.includes('unknown')),
        `the failure must be reported, not swallowed; got ${JSON.stringify(warnings)}`
      )
    } finally {
      console.warn = priorWarn
      globalThis.document = priorDocument
      globalThis.window = priorWindow
      if (priorObserver) globalThis.MutationObserver = priorObserver
    }
  })
})
