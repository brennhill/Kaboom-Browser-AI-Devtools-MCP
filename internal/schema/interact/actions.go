...
var actionSpecs = []ActionSpec{
	...
	{Name: "type", Hint: "Type text into an input or textarea. Hold modifiers to send a shortcut instead of characters (modifiers:['ctrl'] with text 'a' is ctrl+a select-all)", Returns: "Confirmation the text was typed.", Required: []string{"text"}, Optional: []string{"selector", "element_id", "index", "nth", "scope_selector", "frame", "clear", "modifiers"}},
