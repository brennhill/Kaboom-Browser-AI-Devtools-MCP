...
func TestQueryDispatcherArchitectureBoundaries(t *testing.T) {
	t.Parallel()
	...
	if len(paths) > 10 {
		t.Fatalf("queries package has %d Go files; maximum is 10", len(paths))
	}
	...
	for _, forbidden := range []string{"CompleteCommand(", "GetLastPendingQuery("} {
